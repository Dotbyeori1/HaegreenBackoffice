...
export const useUserStore = defineStore('user', {
    ...
    actions: {
        ...
        async logout() {
            await backSideApiService.logout();
            this.role = null;
            this.token = null;
        ...
        async localStorageAuthenticatedRequest() {
            try {
                const response = await backSideApiService.authCheckLocal({token: this.token});
                if (response.data) {
                    localStorage.removeItem('token');
        ...
        async sessionStorageUserAuthenticated() {
            try {
                const response = await backSideApiService.authCheckSession({token: this.sessionToken});
                if (response.data) {
                    sessionStorage.removeItem('token');
